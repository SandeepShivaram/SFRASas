    ...
    if (email) {
        ...
        if (isValidEmailid) {
           // hooksHelper('app.mailingList.subscribe', 'subscribe', [email], function () {});
            var customObject = CustomObjectMgr.getCustomObject('subscriptions', email);    
                var txn = require('dw/system/Transaction');
                txn.wrap(function(){
                    
                   var customObject1 = CustomObjectMgr.createCustomObject('subscriptions', email);
                });
            
            //var obj = CustomObjectMgr.createCustomObject('subscription', email);
            res.json({
                success: true,
